...
<div style="font-size: <?= $_e->fontSize ?>px; line-height: <?= $_e->lineHeight ?>px; font-family: <?= $_e->fontFamily ?>; color: <?= $_e->color ?>">
	...
	<table width="100%" style="border-collapse: collapse; margin: 20px 0;">
		<thead>
			<tr>
            	<th scope="col" style="text-align: left;" width="16%">Return Type</th>
		...
		<tfoot>
			...
			*/
            ?>
			<tr>
				<th scope="row" colspan="4" style="text-align: right;">Return Total</th>
		...
		<tbody>
			...
			foreach ($variables['products'] as $i => $product) {
				...
				?>
				<tr>
                	<td style="text-align: left;"><?= $product['return_type'] ?></td>
					...
					<td style="text-align: right;">$<?= number_format($product['return_quantity'] * $product['product_price'], 2) ?></td>
				</tr>
				<?
			}
			?>
