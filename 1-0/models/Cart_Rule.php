...
class Cart_Rule extends _Model {
	...
	public function get_cart_rule($code, $id_lang) {
		...
		$query = '
			...
			WHERE cart_rule.code = :code
				...
				AND (cart_rule.quantity IS NULL OR cart_rule.quantity > :quantity)
	...
	public function get_membership_discounts() {
		$sql = '
			SELECT cart_rule.*
			FROM cart_rule
				INNER JOIN dahliawolf_v1_2013.membership_level ON cart_rule.id_cart_rule = membership_level.commerce_id_cart_rule
			ORDER BY cart_rule.reduction_percent ASC
		';

		try {
			$data = self::$dbs[$this->db_host][$this->db_name]->exec($sql);

			return $data;
		} catch (Exception $e) {
			self::$Exception_Helper->server_error_exception('Unable to get membership discounts.');
		}
